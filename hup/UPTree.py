...
class UPTree(object):
    ...
    def addTransaction(self, transaction, RTU):
        ...
        for i in range(size):
            for k in range(i+1, size):
                RemainingUtility += transaction[k].utility
            # print("=============")
            # print(vars(self))
            # print("RemainingUtility : ", RemainingUtility)
            item = transaction[i].name
            # print("item : ", item)
    ...
    def addLocalTransaction(self, localPath, pathUtility, mapMinimumItemUtility, pathCount):
        ...
        for i in range(len(localPath)):
            for k in range(1, len(localPath)):
                search = localPath[k]
    ...
    def insertNewNode(self, currentlocalNode, item, nodeUtility):
        ...
        if item in self.mapItemNodes:
            ...
            lastNode = self.mapItemNodes[item]
            lastNode.nodeLink = newNode
            self.mapItemLastNode[item] = newNode
            # print("(i={} count={} nu={}) ".format(self.mapItemLastNode[item].itemID,
            #                                   self.mapItemLastNode[item].count,
            #                                   self.mapItemLastNode[item].nodeUtility))
        else:
